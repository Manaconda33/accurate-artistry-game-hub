...
## Approved implementation amendment 1.4 - Active slice sequence and mobile race controls

Approved August 16, 2026 with product-owner authorization to begin Slice 3. The active delivery sequence established by the repository status is authoritative: Slice 3 is AI Waypoint Navigation & Eight-Racer Grid; Character Selection & Avatar Ingestion moves to Slice 4; Items move to Slice 5; final presentation and hardening remain Slice 6. Slice 3 adds touch driving controls for mobile sessions. Touch controls must be rendered only when the browser reports a coarse primary pointer with no hover capability and must support steering, acceleration, brake/reverse, drift, rear view, and recovery without removing keyboard controls. Desktop/fine-pointer sessions must not show the touch overlay. Because the item system is not implemented until Slice 5, AI-004 item use remains dependency-blocked and is not a Slice 3 completion claim. This amendment supersedes the earlier statement that mobile touch controls are outside the vertical slice and the original delivery numbering in sections 35.4-35.6 where they conflict with the active repository sequence.
